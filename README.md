...
在三维地图或场景应用中，相机参数对于控制观察视角和导航非常重要。通过该组件，用户可以实时查看和调整相机的位置和姿态，并提供了重置和复制相机参数的功能。

![Alt text](./mdimg/相机参数1.png)

[官网博客相机参数介绍](https://www.esri.com/arcgis-blog/products/js-api-arcgis/3d-gis/arcgis-api-for-javascript-camera-intro/)

* 该组件提供：

实时显示相机的位置、朝向和倾斜角度等参数。

重置相机参数：将相机参数重置为初始状态。

复制相机参数：将相机参数以文本格式复制到剪贴板。

场景坐标系转换：将场景的坐标转化到其他坐标系的值。

![Alt text](./mdimg/相机参数2.png)
...
位置定位的功能，它包含一个搜索框和一个定位按钮。用户可以在搜索框中输入地址，然后从下拉列表中选择一个地址进行定位。

在搜索框中输入地址时，组件会调用百度地图的API，获取匹配的地址建议，并将建议显示在下拉列表中供用户选择。

地址选择：当从下拉列表中选择一个地址时，点击定位按钮时，组件会根据选中的地址信息，将地图视图定位到对应的位置。同时，组件会在地图上生成一个点图形，表示定位的位置。
